...
int init_array(dyn_array *the_array, int size)
{
    ...
    the_array->all_reviews = calloc(size, sizeof(review));

    if (!the_array->all_reviews) return 1;
...
int add_review(dyn_array *the_array)
{
    ...
    if (the_array->index == the_array->size){
        int new_size = 2*the_array->size;
        the_array->size = new_size;

        new_reviews_array = calloc(new_size,sizeof(review));

        if (!new_reviews_array) return 1;
        
        // original pointer
        review *old_array = the_array->all_reviews;
        // new pointer
        the_array->all_reviews = new_reviews_array;


        for (int i = 0; i < the_array->size; i++){
            new_reviews_array->score = (old_array+i)->score;
            strcpy(new_reviews_array->name, (old_array+i)->name);
            new_reviews_array++;
        }
        free(old_array);
